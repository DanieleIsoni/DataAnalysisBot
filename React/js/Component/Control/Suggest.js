...
class Suggest extends React.Component{
    ...
    clearHistory(){
        if (typeof(Storage) !== "undefined") {
            sessionStorage.setItem('suggestion', JSON.stringify([]));
        }
    }
    ...
    render(){
        ...
        return (
            <div className="suggest" id="suggest" ref={sug => this.suggest = sug} style={{display: (this.props.type == "NL" && this.props.input.length > 0) ? "block" : "none" }}>
                {suggest_list}
                <span className="clear-history" style={{display: (this.props.type == "NL" && this.props.suggest.size > 0) ? "block" : "none" }} onClick={() => this.clearHistory()}>Clear<i class="material-icons">close</i></span>
            </div>
        );
    }
}
